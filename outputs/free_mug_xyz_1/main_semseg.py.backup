...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        for data, seg in test_loader:
            ...
            loss = criterion(seg_pred.view(-1, 7), seg.view(-1,1).squeeze())
...
def test(args, io):
    ...
    if (args.test_area == 'all') or (test_area == args.test_area):
        ...
        for data, seg in test_loader:
            data, seg = data.to(device), seg.to(device)
            data = data[:,:,:6] ## data changed (16, 4096, 6)
            ...
            visualization(args.visu, args.visu_format, data, seg, pred, benchmark_seg_colors) 
...
def test_embeddings(args, io):
    ...
    if (args.test_area == 'all') or (test_area == args.test_area):
        # test_loader = DataLoader(S3DIS(partition='test', num_points=args.num_points, test_area=test_area),
        #                          batch_size=args.test_batch_size, shuffle=False, drop_last=False)
        test_loader = DataLoader(BENCHMARK(partition='train', num_points=args.num_points),
        ...
        for i, (data, seg) in enumerate(test_loader):
            ...
            ## conv 8 embedding
            conv8 = activation['conv8'].permute(0, 2, 1).contiguous()
            conv8 = conv8.detach().cpu().numpy()
            ...
            test_true_seg.append(seg_np)
            test_pred_seg.append(pred_np)
            print(f"{i} 7d shape {seg_pred_np.shape} conv8 shape {conv8.shape} pred shape {pred_np.shape} gt shape {seg_np.shape}")

            with h5py.File('emb_for_mlp/emb_for_mlp_'+str(i).zfill(4)+'.h5', 'w') as f:
                f.create_dataset('embeddings', data = seg_pred_np)
                f.create_dataset('conv8_embeddings', data = conv8)
                f.create_dataset('predictions', data = pred_np)
                f.create_dataset('labels', data = seg_np)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='BENCHMARK', metavar='N',
                        choices=['S3DIS, BENCHMARK'])
